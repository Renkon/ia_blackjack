...
class Aumentador:
    ...
    def preparar_procesamiento(self, source):
        if not os.path.exists(source / "tmp" / "src"):
            os.makedirs(source / "tmp" / "src")

    def procesar_imagen(self, label, source):
        print("Iniciando procesamiento de " + str(source))

        images_dir = source.parent.parent

        dest = images_dir / "tmp" / "src" / (str(label) + ".png")
        if not os.path.exists(dest):
            self.__transformar_imagen_inicial(str(label), source, dest)

        print("Validada existencia de source preprocesado")
...
#
    ...
    def __transformar_imagen_inicial(self, label, source, dest):
